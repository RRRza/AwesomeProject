...
export default function TourFlatList(props) {
    ...
    return (
        <View style={props.style}>
            <Text style={{ fontSize: 20, paddingLeft: 10 }}>Tour with FlatList</Text>
            <Text style={{ color: 'gray', marginVertical: 5, paddingLeft: 10 }}>Let find out what most interesting things</Text>
            <FlatList
                horizontal={true}
                data={onlineTours}
                renderItem={
                    ({ item, index }) => {
                        ...
                        return (
                            <View style={{ marginRight: 10 }}>
                                <Image style={{ width: 250, height: 150, borderRadius: 20 }} source={{ uri: item.uri }} />
